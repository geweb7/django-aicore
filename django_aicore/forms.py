...
class ProviderForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "base_url": forms.URLInput(attrs={"class": "form-control form-control-sm",
    ...
    def clean(self):
        ...
        if not base_url or not dialect:
            return cleaned

        is_root, path = api_url_is_root(base_url)

        if dialect == Provider.DIALECT_OPENAI and is_root:
            self.add_error("base_url", (
                ...
                f"«{base_url.rstrip('/')}/chat/completions» (для эмбеддингов — "
                f"«{base_url.rstrip('/')}/embeddings»). Путь достраивает сам только диалект «Gemini»."
            ))
...
class AIModelForm(forms.ModelForm):
    class Meta:
        model = AIModel
        fields = ["description", "api_key", "model", "role", "priority",
