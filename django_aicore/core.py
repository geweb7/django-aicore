...
def stack_roots():
    """Корневые пакеты всех кадров стека, по порядку и без повторов.

    Нужны в сообщении об отказе: «приложение не определилось» без списка того, что слой
    видел, неотличимо от «слой сломался», и чинить по такому сообщению нечего.
    """
    roots, frame = [], sys._getframe(1)
    while frame is not None:
        root = (frame.f_globals.get("__name__") or "").split(".", 1)[0]
...
def resolve_app(task=None):
    ...
    Слаг задачи — вторая опора на случай, когда стек приложения не называет.

    **Не определилось — вызов не делается.** Расход на AI обязан быть отнесён к
    приложению; неотнесённый вызов оплачивается из общего кошелька и не виден ни в одной
    сводке. Дефолт вроде «django_aicore» тут хуже отказа: он выглядит как ответ, ложится
    в журнал наравне с настоящими приложениями и тихо портит учёт — ровно так весь трафик
    легаси-`call()` годами уходил в OpenRouter под именем слоя.
    """
    # Ключ без точки приложения не называет: взять его целиком значило бы завести в
    # учёте приложение, которого нет, — а отличить такую строку от настоящей потом нечем.
    app = calling_app() or (task.key.split(".", 1)[0] if task and "." in task.key else "")
    if not app:
        raise AIUnavailableError(
            "Вызов AI отклонён: не удалось определить приложение-инициатор, а расход "
            "обязан быть отнесён к приложению — иначе он не виден ни в одной сводке "
            "и оплачивается из общего кошелька.\n"
            f"Корневые пакеты в стеке вызова: {', '.join(stack_roots())} — среди них нет "
            f"ни одного вне слоя (не считаются: {', '.join(sorted(NON_APP_ROOTS))}).\n"
            "Зовите AI из кода приложения либо через call_task с ключом вида "
            "«приложение.сценарий» — первый сегмент ключа и есть приложение."
        )
    return app
